...
<script>
  ...
  function handleClick() {
    // Zde můžete vložit kód, který zavolá na dané telefonní číslo
    // Například:
    window.location.href = 'tel:+420 380 741 055';
  }
</script>
...
<nav class="navbar fixed top-0 left-0 w-full z-10 bg-opacity-50 backdrop-filter backdrop-blur-md p-4">
  <div class="container mx-auto flex justify-between items-center">
    ...
    <ul class="flex items-center space-x-8">
      ...
      <li><a href="/cenik" class="text-white hover:bg-green-500 hover:text-white transition duration-300 ease-in-out rounded-lg py-1 px-4 bg-green-400">Ceník</a></li>
      <li><a href="/deti" class="text-white hover:bg-purple-500 hover:text-white transition duration-300 ease-in-out rounded-lg py-1 bg-blue-400">Pro děti</a></li> 
    </ul>
    <button class=" animate-btn bg-green-500 text-white font-bold  ml-16 py-2 px-4 rounded mr-36" on:click={handleClick}>
